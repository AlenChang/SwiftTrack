...
complex<double> Postprocessor::LeastSquare(){
    ...
    complex<double> beta1 = MatrixUtil::DotSum(prev_irs_signal_.conjugate(), irs_signal_);
    complex<double> beta2 = MatrixUtil::DotSum(prev_irs_signal_.conjugate(), prev_irs_signal_);
...
void Postprocessor::CalcPhase(bool is_moving) {
    ...
    double phase_unwrapped = swifttrack_history_.phase_history_.back() + phase_diff;
...
void Postprocessor::PhaseTransform() {
    double velocity = -swifttrack_history_.phase_history_.back() * C / (4 * M_PI * FC * T);
